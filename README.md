<img src="https://capsule-render.vercel.app/api?type=waving&color=0:58A6FF,100:1F6FEB&height=140&section=header" width="100%"/>

<h1 align="center">Perez's Learning Branch</h1>

<p align="center">
  <img src="https://github.com/Rust-Frog/Teaching-My-Peers-Github-and-Git/actions/workflows/java-compile-check.yml/badge.svg?branch=perez-learning" alt="Build Status"/>
  <img src="https://img.shields.io/badge/Branch-perez-learning-58A6FF?style=flat-square&logo=git&logoColor=white" alt="Branch"/>
  <img src="https://img.shields.io/badge/Last_Updated-2026---03---21%2013:53%20UTC-2EA44F?style=flat-square&logo=clock&logoColor=white" alt="Updated"/>
</p>

---

## ✅ Great Job! Your Code Compiles Perfectly!

> 🎉 **I'm so proud of you!** Your code compiled with zero errors.
>
> Every green badge means you're one step closer to mastering programming.
> Keep up the amazing work — you're doing great!

```
✅ COMPILATION SUCCESSFUL
═══════════════════════════════════════════════════
All Java files compiled without errors. Great job!
═══════════════════════════════════════════════════
```

---

## 📝 How to Solve Exercises

Pick whichever method works best for you:

<details open>
<summary><strong>🌐 Option 1 — Edit on GitHub (Easiest, No Setup)</strong></summary>

<br/>

1. Navigate to your file in the [`exercises/`](https://github.com/Rust-Frog/Teaching-My-Peers-Github-and-Git/tree/perez-learning/exercises) folder
2. Click the **✏️ pencil icon** (top right of the file)
3. Fix the code right in the browser
4. Scroll down and click **"Commit changes"**
5. Done! CI will check your code automatically

</details>

<details>
<summary><strong>💻 Option 2 — Use github.dev (VS Code in Your Browser)</strong></summary>

<br/>

1. Go to [this repo](https://github.com/Rust-Frog/Teaching-My-Peers-Github-and-Git)
2. Make sure you're on branch `perez-learning`
3. Press the **`.`** key (period) on your keyboard — this opens **VS Code in your browser**!
4. Edit the file, save it (`Ctrl+S`)
5. Click the **Source Control** icon (left sidebar) → stage, commit, and push

> 💡 Or go directly: [github.dev/Rust-Frog/Teaching-My-Peers-Github-and-Git/tree/perez-learning](https://github.dev/Rust-Frog/Teaching-My-Peers-Github-and-Git/tree/perez-learning)

</details>

<details>
<summary><strong>🖥️ Option 3 — Clone Locally (Full Dev Setup)</strong></summary>

<br/>

```bash
# Clone the repo
git clone https://github.com/Rust-Frog/Teaching-My-Peers-Github-and-Git.git
cd Teaching-My-Peers-Github-and-Git

# Switch to your branch
git checkout perez-learning

# Open in your editor, fix the code, then:
git add .
git commit -m "Fix: describe what you fixed"
git push origin perez-learning
```

</details>

---

## 📋 Your Exercises

| # | Folder | File | Status |
|:--|:-------|:-----|:------:|
| 1 | `exercises/exercise01_hello_world` | [`HelloWorld.java`](https://github.com/Rust-Frog/Teaching-My-Peers-Github-and-Git/blob/perez-learning/exercises/exercise01_hello_world/HelloWorld.java) | — |
| 2 | `exercises/exercise01_naming` | [`FavoriteMovie.java`](https://github.com/Rust-Frog/Teaching-My-Peers-Github-and-Git/blob/perez-learning/exercises/exercise01_naming/FavoriteMovie.java) | — |

---

## 📌 Quick Commands

| Command | What It Does |
|:---|:---|
| `git checkout perez-learning` | Switch to your branch |
| `git pull origin perez-learning` | Get latest changes |
| `javac YourFile.java` | Compile locally |
| `java YourFile` | Run your code |
| `git add . && git commit -m "msg"` | Save your work |
| `git push origin perez-learning` | Upload to GitHub |

---

<p align="center">
  <em>This README is automatically updated by GitHub Actions after every push.</em><br/>
  <strong>Keep coding, keep learning! 🚀</strong>
</p>

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:58A6FF,100:1F6FEB&height=100&section=footer" width="100%"/>
